...
class Fiber(ABC):
    ...
    def compute_modes_fields(self, grid: Grid) -> None:
        ...
        for i in range(self._N_modes):
            n, m = self._neff_hnm[i, 2], self._neff_hnm[i, 3]
    ...
    @abstractmethod
    def modes_coupling_matrix(self) -> None:
        pass
    ...
    @property
    def stored_modes_fields(self):
        return True if self._modes is not None else False
    ...
    def plot_coupling_matrix(matrix, cmap: str = 'gray', complex: bool = False, complex_hsv: bool = False):
        if complex:
            if complex_hsv:
                ...
                ax.set_title(f"Fiber coupling matrix (complex)")
                return (fig, ax, pl)
            else:
                ...
                axs[0].set_title(f"Fiber coupling matrix (amplitude)")
                axs[1].set_title(f"Fiber coupling matrix (phase)")
            ...
            ax.set_xlabel("Input mode index")
            ax.set_ylabel("Output mode index")
            ax.set_title(f"Fiber coupling matrix (amplitude)")
...
class StepIndexFiber(GrinFiber):
    ...
    def __init__(self, radius: float = 6.5e-6, wavelength: float = 1064e-9, n1: float = 1.465, n2: float = 1.445) -> None:
        ...
        super().__init__(radius, wavelength, n1, n2)
        self._mode_type = StepIndexLPMode
